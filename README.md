...
├── styles.css      # 전체 스타일 (다크 테마, 글래스모피즘, 반응형)
├── script.js       # 네트워크 캔버스 애니메이션 + 다국어(translations/setLanguage)
...
> 이때 **이메일(MX) 레코드가 누락되면 `appfactory.publish@gmail.com` 등 메일 수발신이 중단**될 수 있습니다.
